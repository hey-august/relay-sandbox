"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.createRoomSessionStreamObject = exports.RoomSessionStreamAPI = void 0;
const tslib_1 = require("tslib");
const redux_1 = require("../redux");
const BaseComponent_1 = require("../BaseComponent");
class RoomSessionStreamAPI extends BaseComponent_1.BaseComponent {
    constructor(options) {
        super(options);
        this._payload = options.payload;
    }
    get id() {
        return this._payload.stream.id;
    }
    get roomId() {
        return this._payload.room_id;
    }
    get roomSessionId() {
        return this._payload.room_session_id;
    }
    get state() {
        return this._payload.stream.state;
    }
    get duration() {
        return this._payload.stream.duration;
    }
    get url() {
        return this._payload.stream.url;
    }
    get startedAt() {
        if (!this._payload.stream.started_at)
            return undefined;
        return new Date(this._payload.stream.started_at * 1000);
    }
    get endedAt() {
        if (!this._payload.stream.ended_at)
            return undefined;
        return new Date(this._payload.stream.ended_at * 1000);
    }
    /** @internal */
    setPayload(payload) {
        this._payload = payload;
    }
    stop() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            yield this.execute({
                method: 'video.stream.stop',
                params: {
                    room_session_id: this.getStateProperty('roomSessionId'),
                    stream_id: this.getStateProperty('id'),
                },
            });
        });
    }
}
exports.RoomSessionStreamAPI = RoomSessionStreamAPI;
const createRoomSessionStreamObject = (params) => {
    const stream = (0, redux_1.connect)({
        store: params.store,
        Component: RoomSessionStreamAPI,
    })(params);
    return stream;
};
exports.createRoomSessionStreamObject = createRoomSessionStreamObject;
//# sourceMappingURL=RoomSessionStream.js.map