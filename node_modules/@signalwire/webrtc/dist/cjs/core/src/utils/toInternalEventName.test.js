...
const toInternalEventName_1 = require("./toInternalEventName");
describe('toInternalEventName', () => {
    it('should use the EVENT_NAMESPACE_DIVIDER constant for namespacing the event', () => {
        const internalEvent = (0, toInternalEventName_1.toInternalEventName)({
            event: 'video.room.started',
            namespace: '1234',
        });
        expect(internalEvent).toEqual(`1234${constants_1.EVENT_NAMESPACE_DIVIDER}video.room.started`);
    });
    it('should transform public events into internal without namespace', () => {
        ...
        events.forEach(([input, output]) => {
            expect((0, toInternalEventName_1.toInternalEventName)({ event: input })).toEqual(output);
            // @ts-ignore
            expect((0, toInternalEventName_1.toInternalEventName)({ event: input, namespace: null })).toEqual(output);
            expect((0, toInternalEventName_1.toInternalEventName)({ event: input, namespace: '' })).toEqual(output);
            expect((0, toInternalEventName_1.toInternalEventName)({ event: input, namespace })).toEqual(`${namespace}:${output}`);
        });
    });
...
//# sourceMappingURL=toInternalEventName.test.js.map