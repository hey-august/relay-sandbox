...
import { initAction, destroyAction, sessionReconnectingAction, sessionDisconnectedAction, reauthAction, sessionForceCloseAction, } from './actions';
import { sessionActions } from './features';
import { authErrorAction, authSuccessAction, authExpiringAction, } from './actions';
import { AuthError } from '../CustomErrors';
import { createRestartableSaga } from './utils/sagaHelpers';
export function* initSessionSaga({ initSession, sessionEmitter, userOptions, channels, }) {
    var _a;
     ...
     */
    const sessionStatusTask = yield fork(sessionStatusWatcher, {
        session,
        sessionEmitter,
        sessionChannel,
        userOptions,
...
export function* reauthenticateWorker({ session, token, sessionEmitter, }) {
    try {
...
export function* sessionStatusWatcher(options) {
        ...
        while (true) {
            ...
            switch (action.type) {
                ...
                case authErrorAction.type: {
                    yield fork(sessionAuthErrorSaga, Object.assign(Object.assign({}, options), { action }));
                    break;
                }
...
export function* sessionAuthErrorSaga(options) {
...
export default (options) => {
    return function* root({ userOptions, channels, }) {
        ...
        while (true) {
             ...
             */
            try {
                yield call(initSessionSaga, Object.assign(Object.assign({}, options), { userOptions,
                    channels }));
            }
            catch (error) {
