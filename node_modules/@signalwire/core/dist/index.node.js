...
__export(src_exports, {
  ...
  LOCAL_EVENT_PREFIX: () => LOCAL_EVENT_PREFIX,
  ...
  VertoSubscribe: () => VertoSubscribe,
  WEBRTC_EVENT_TYPES: () => WEBRTC_EVENT_TYPES,
  ...
  isSATAuth: () => isSATAuth,
  isWebrtcEventType: () => isWebrtcEventType,
...
function* initSessionSaga({
  ...
  if ((_a = userOptions.workers) == null ? void 0 : _a.length) {
    try {
      const effects = userOptions.workers.map((saga) => {
        return (0, import_effects3.call)(createRestartableSaga(saga));
      });
      customTasks = yield (0, import_effects3.all)(effects);
    } catch (error) {
      getLogger().error("Error running custom workers", error);
    }
  }
  yield (0, import_effects3.fork)(sessionChannelWatcher, {
    session,
    sessionChannel,
    swEventChannel
  });
  const sessionStatusTask = yield (0, import_effects3.fork)(sessionStatusWatcher, {
    session,
    sessionEmitter,
    sessionChannel,
    userOptions
  ...
  session.connect();
  yield (0, import_effects3.take)(destroyAction.type);
  session.disconnect();
  yield (0, import_effects3.take)(sessionDisconnectedAction.type);
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      session.token = token;
      yield (0, import_effects3.call)(session.reauthenticate);
      yield (0, import_effects3.put)(sessionActions.connected(session.rpcConnectResult));
...
function* sessionStatusWatcher(options) {
  ...
  try {
    while (true) {
      const action = yield (0, import_effects3.take)([
      ...
      switch (action.type) {
        ...
        case authErrorAction.type: {
          yield (0, import_effects3.fork)(sessionAuthErrorSaga, __spreadProps(__spreadValues({}, options), {
            action
          }));
          break;
        }
        ...
        case reauthAction.type: {
          yield (0, import_effects3.fork)(reauthenticateWorker, {
            session,
...
var rootSaga_default = (options) => {
  return function* root({
    ...
    while (true) {
      const action = yield (0, import_effects3.take)([initAction.type, reauthAction.type]);
      if ((_a = action == null ? void 0 : action.payload) == null ? void 0 : _a.token) {
        userOptions.token = action.payload.token;
      }
      try {
        yield (0, import_effects3.call)(initSessionSaga, __spreadProps(__spreadValues({}, options), {
          userOptions,
          channels
        }));
      } catch (error) {
...
var configureStore2 = (options) => {
  ...
  const sessionChannel = (0, import_core2.channel)();
  const channels = {
    swEventChannel,
    sessionChannel
  };
...
var executeActionWorker = function* (options) {
  ...
  try {
    ...
    const response = yield (0, import_effects4.call)(session.execute, message);
...
var import_core3 = require("@redux-saga/core");
...
var createSwEventChannel = () => (0, import_core3.multicastChannel)();
var createSessionChannel = () => (0, import_core3.channel)();
